...
export const RequestTable = () => {
  ...
  const COLUMNS = useMemo (() => [
      ...
      <Tooltip title = "Delete"><DeleteIcon style={{ color: 'red' }} onClick = {() => handleDeleteClick(row)} /></Tooltip>
      <Tooltip title="Edit"><EditIcon style={{ color: 'grey' }} onClick={() => handleEditClick(row)}/></Tooltip>
      </>)},
  ...
  const handleConfirmDelete = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:5201/api/Requests/${rowToDelete.original.requestId}`, {
      method: 'DELETE',
      });
      ...
      setData((prevState) => prevState.filter((item) => item.requestId !== rowToDelete.original.requestId));
  ...
  const handleUpdateRequest = async () => {
	try {
	  const response = await fetch(`http://localhost:5201/api/Requests/${editRow.original.requestId}`, {
		method: "PUT",
		headers: {
		  "Content-Type": "application/json"
		},
  ...
  return (
    <>
		...
		<Dialog open={editRow !== null} onClose={() => setEditRow(null)} fullWidth maxWidth="xs"> 
			...
			<DialogContent>
				<Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
					<TextField
						...
						onChange={(e) => setUpdatedRequest({...updatedRequest, vendor_Name: e.target.value})}
					/>
					<TextField
						margin="dense"
						label="Vendor Mailing Address"
						...
						onChange={(e) => setUpdatedRequest({...updatedRequest, vendor_Mailing_Address: e.target.value})}
					/>
					{/* Add more text fields for other columns */}
				</Box>
			</DialogContent>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'flex-end' }}>
            <Button onClick={() => setCreateModalOpen(true)}>Add New Request</Button>
        </Box>
    </>
  );
};
